...
처리 흐름:
  Step 1. 고정 축 5개 로드 (config.GAP_AXES_FIXED)
...
def _load_fixed_axes() -> dict:
    """config에서 고정 축 5개를 로드한다."""
    return dict(config.GAP_AXES_FIXED)


# ── Step 2. 동적 축 생성 ──────────────────────────────────────────────────────

def _generate_dynamic_axes(limitations_text, fixed_axes, research_question):
    """
    수집된 한계점 전체를 보고 고정 축으로 커버되지 않는
    도메인 특화 패턴을 LLM이 찾아 동적 축으로 제안한다.

    Returns:
        [{"name": str, "label": str, "description": str, "reason": str}, ...]
    """
    fixed_summary = "\n".join(
        f"  - {k}: {v['description']}" for k, v in fixed_axes.items()
    )
    min_papers = config.GAP_AXES_DYNAMIC_MIN_PAPERS
    max_axes   = config.GAP_AXES_DYNAMIC_MAX

    prompt = f"""You are a research gap analyst covering ALL scientific domains
(AI, life sciences, chemistry, engineering, medicine, social sciences, etc.).
...
by the 5 fixed axes. These should be recurring themes appearing in {min_papers}+ papers.

Rules:
- Propose {max_axes} extra axes at most. If no clear pattern exists, return empty list.
...
def _build_final_axes(fixed_axes, dynamic_axes):
    """
    고정 축 + 동적 축을 합쳐 최종 축 dict를 반환한다.
    key   -> axis name (snake_case)
    value -> {"label": str, "description": str, "type": "fixed"|"dynamic"}
    """
    final = {}
...
def _classify_limitations_batch(limitations, final_axes):
    """
    모든 limitation을 BATCH_SIZE 단위로 나눠 LLM에 배치 전송한다.
    limitation 개수만큼 LLM을 호출하던 기존 방식 대비 호출 횟수를 대폭 줄인다.

    Returns:
        {limitation_index: axis_name, ...}
    """
    BATCH_SIZE = 20
    axis_mapping = {}
    ...
    axes_block = "\n".join(
        f"  {k}: {v['description']}" for k, v in final_axes.items()
    )
    axis_keys = list(final_axes.keys())

    batches = [
        limitations[i: i + BATCH_SIZE]
        for i in range(0, len(limitations), BATCH_SIZE)
    ]

    for batch_idx, batch in enumerate(batches):
        items_block = "\n".join(
            f'  [{i}] "{lim.claim}"'
            for i, lim in enumerate(batch)
        )
...
"""
        messages = [
            {"role": "system", "content": (
                "You are a research limitation classifier. "
                "Always respond in valid JSON."
            )},
            {"role": "user", "content": prompt},
        ]
...
def _generate_gap_for_axis(ax_key, ax_info, ax_lims, research_question):
    """
    하나의 축에 대해 GAP statement + 상세 설명 + 제안 연구 주제를 생성한다.

    Returns:
        {"gap_statement": str, "elaboration": str, "proposed_topic": str} | None
    """
    claims_block = "\n".join(
        f"  - [Paper {lim.paper_id}] {lim.claim}"
        for lim in ax_lims[:8]
    )
    quotes_block = "\n".join(
        f'  - "{lim.evidence_quote}"'
        for lim in ax_lims[:5]
        if lim.evidence_quote
    )
...
def gap_infer_node(state: AgentState) -> AgentState:
    """
    GAP Inference Node (고도화)

    Step 1. 고정 축 5개 로드
    Step 2. 동적 축 생성 (LLM 1회)
    Step 3. 최종 축 확정
    Step 4. 전체 limitations 배치 분류 (LLM 호출 최소화)
    Step 5. 축별 GAP + 제안 주제 생성
    """
    print("\n💡 GAP Inference Node")
    ...
    # ── Step 2 ──────────────────────────────────────────────────
    all_claims_text = "\n".join(
        f"[{lim.paper_id}] {lim.claim}" for lim in limitations
    )
    print(f"  🔄 동적 축 생성 중 (limitations {len(limitations)}개 분석)...")
    dynamic_axes = _generate_dynamic_axes(
        all_claims_text, fixed_axes, research_question
    )
    ...
    for ax_key, ax_lims in sorted(active_axes.items(), key=lambda x: -len(x[1])):
        ax_info = final_axes.get(ax_key, {
            "label": ax_key, "description": "", "type": "fixed"
        })
        result = _generate_gap_for_axis(
            ax_key, ax_info, ax_lims, research_question
        )
        if result is None:
            continue
        ...
        gaps.append(GapCandidate(
            ...
            supporting_quotes = [
                lim.evidence_quote for lim in ax_lims if lim.evidence_quote
            ][:5],
        ))
    ...
    state["trace"]["axes_used"]          = list(final_axes.keys())
    state["trace"]["dynamic_axes"]       = [ax["name"] for ax in dynamic_axes]
    state["trace"]["axis_distribution"]  = {
        k: len(v) for k, v in axis_groups.items()
    }
